...
md.predictvars = np.array(['Y-CO2','Y-H2','Y-N2','Y-CO','Y-O2','Y-H2O','Y-'+md.fuel,'Y-OH','Y-CH2O','Y-HO2',
                           ...
                           'T','RHO','DIFF','VISC'])
src_idx = torch.zeros(len(md.trainvars), dtype=torch.int64)        

for i, v in enumerate(md.trainvars):
    if v.startswith('Y-'):
        v = v[2:]
    v = "SRC_" + v
    locs = (md.predictvars == v).nonzero()[0]
    if len(locs) > 0:
        src_idx[i] = locs[0]
    else:
        src_idx[i] = -1
...
srclossfunc = mrn.sgn_loss_mpar(src_idx, 1e3)
...
md.trnloss[label], md.tstloss[label] = mrn.train_net_sherpa(get_invars_flt(trn_raw),trn_raw['out'],
                                                            ...
                                                            flt_net, lossfunc=lossfunc,
                                                            srclossfunc=srclossfunc, scalers=md.scalers,
                                                            calc_srcloss=True, dev=dev,
